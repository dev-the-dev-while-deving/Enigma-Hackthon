...
env = gym.make("highway-v0", render_mode="human", config={
    ...
    "vehicles_count": 50,         # Heavy dense traffic
    "vehicles_density": 1.5,      # High density forces braking waves
    "initial_spacing": 2.0,       # Tight spacing creates the "traffic jam" feel
    "collision_reward": -50,   
    "high_speed_reward": 0.5,     # Lower reward to prioritize safety over 110km/h
    "right_lane_reward": 0.1,  
    "reward_speed_range": [20.0, 25.0], # Reduced speeds (70-90 km/h) typical of dense braking traffic
    "observation": {
        "type": "Kinematics",
        "noise": 0.05,            # Mild uncertainty
    ...
    while not (done or truncated):
        ...
        for v in env.unwrapped.road.vehicles:
            if v is not ego:
                ...
                # Check Same Lane
                if abs(dy) < 2.0:
                    if 0 < dx < 18.0:  # 18m buffer for moderate braking in 70-90 km/h traffic
                        crash_ahead = True
