...
def evaluate_trajectory(mean_traj, epi_std, obs, safety_margin=0.15):
    ...
    # We want to maximize progress in any direction (for intersection turning/crossing)
...
env = gym.make("intersection-v0", render_mode="human", config={
    ...
    while not (done or truncated) and step < 100:
        ...
        for v in env.unwrapped.road.vehicles:
            if v is not ego:
                ...
                # If any vehicle is extremely close in the intersection, brake immediately to avoid clipping
                if dist < 5.5:
                    imminent_crash = True
