...
env = gym.make("highway-v0", render_mode="human", config={
    ...
    "vehicles_count": 40,         
    "vehicles_density": 1.8,      # High density promotes tight grouping
    "initial_spacing": 1.5,       # Tight spawns cause "walls" of traffic
    "collision_reward": -50,   
    "high_speed_reward": 1.2,     # Maintain incentive to bypass the slow wall
    "right_lane_reward": 0.1,     # Re-enable slight bias so the cluster is naturally an obstacle
    "reward_speed_range": [20.0, 31.0], # Massive differential (70-110 km/h) forces slow cars to clump together
    "observation": {
        "type": "Kinematics",
        "noise": 0.07,            # Moderate uncertainty
    ...
    while not (done or truncated):
        ...
        for v in env.unwrapped.road.vehicles:
            if v is not ego:
                ...
                # Check Same Lane
                if abs(dy) < 2.0:
                    if 0 < dx < 22.0:  # 22m buffer for high speeds in 3-lane traffic
                        crash_ahead = True
                
                # Check Left Lane
                if -6.0 < dy <= -2.0:
                    if abs(dx) < 12.0: # Increased blind spot buffer
                        blocked_left = True
