...
env = gym.make("highway-v0", render_mode="human", config={
    ...
    "vehicles_count": 25,         # Medium traffic to ensure both slow cars and gaps exist
    "vehicles_density": 1.2,      # Medium density
    "initial_spacing": 2.2,       # Balanced spawning to create viable overtaking opportunities
    "collision_reward": -50,   
    "high_speed_reward": 1.5,     # Increased reward to heavily incentivize 100+ km/h
    "right_lane_reward": 0.1,  
    "reward_speed_range": [28.0, 31.0], # Strongly reward 100-110 km/h, making 70-85 km/h a "penalty"
    "observation": {
        "type": "Kinematics",
        "noise": 0.06,            # Mild uncertainty
    ...
    while not (done or truncated):
        ...
        for v in env.unwrapped.road.vehicles:
            if v is not ego:
                ...
                # Check Same Lane
                if abs(dy) < 2.0:
                    if 0 < dx < 25.0:  # Increased to 25m because of 110 km/h speeds!
                        crash_ahead = True
